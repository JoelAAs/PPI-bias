...
def get_cumulative_sum(df, value_column, cumulative_columns, greater=True, min_samples=200):
    ...
    cumulative_sum = np.zeros(len(cumulative_columns))
    cumulative_na = np.zeros(len(cumulative_columns))
    ...
    for threshold in bins:
        while (i < len(values) and threshold <= values[i]) or i < min_samples:
            i += 1
        if previous != i:
            cumulative_sum += np.nansum(measurement_matrix[previous:i], axis=0)
            cumulative_na += np.nansum(na_matrix[previous:i], axis=0)
            previous = i
            rows[j] = {
                "limit": value_column,
                "value": (threshold if greater else -threshold),
            } + {
                f"sum_{c}": v for c,v in zip(cumulative_sum, cumulative_sum)
            } + {
                f"non_na_pairs_{c}": v for c, v in zip(cumulative_na, cumulative_sum)
            }
            j += 1
