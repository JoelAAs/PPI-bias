...
def main():
    ...
    coessentiality_uniprotids = set(pd.read_csv(snakemake.input.oe_lof_category, sep="\t")["uniprot_id"])
    neg_df = neg_df[
        (neg_df["bait"].isin(coessentiality_uniprotids)) &
        (neg_df["prey"].isin(coessentiality_uniprotids))
    ]
    pos_df = pos_df[
        (pos_df["bait"].isin(coessentiality_uniprotids)) &
        (pos_df["prey"].isin(coessentiality_uniprotids))
    ]
