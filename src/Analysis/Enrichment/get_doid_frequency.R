...
degree_df <- merge(degree_df, gene_df, on="entrez_id", all=TRUE)
degree_df %>%
    slice_max(!!sym(val_column), n=n_top) -> degree_df
...
degree_df <- merge(doids, degree_df, on="entrez_id", all=TRUE)
degree_df %>%
    group_by(doid) %>%
    summarize(
...
write.table(doid_count,doid_annotation_file,sep="\t", row.names = FALSE)
