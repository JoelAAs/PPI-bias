...
rule cdhit_redudance_between_subsets:
    ...
    shell:
        ...
        sed -nE 's/.*>([A-Za-z0-9-]+)....*%$/\1/p' -o {output.sim_train}.clstr > {output.redundant_proteins}
        sed -nE 's/.*>([A-Za-z0-9-]+)....*%$/\1/p' -o {output.sim_validation}.clstr >> {output.redundant_proteins}
        sed -nE 's/.*>([A-Za-z0-9-]+)....*%$/\1/p' -o {output.sim_test}.clstr >> {output.redundant_proteins}
        """
