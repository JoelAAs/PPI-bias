...
def build_flow_graph(target_bait, target_prey, edge_G, node_idx):
    F = nx.DiGraph()
    bp_index = {
        (0, b_idx): i for i, b_idx in enumerate(target_bait.keys())
    }
    bp_index.update({
...
def main():
    ...
    validation_graphs, test_graphs = remove_all_nonovelapping_nodes(remaining_graphs[0], remaining_graphs[1], 0.5)
    G_pos_validation, G_neg_validation = back_and_forth_max_flow(validation_graphs[0], validation_graphs[1])
    
    G_pos_test, G_neg_test = test_graphs
    
    nx.write_edgelist(G_pos_train, snakemake@output.train_pos , delimiter="\t", data=False)
    nx.write_edgelist(G_neg_train, snakemake@output.train_neg , delimiter="\t", data=False)
    
    nx.write_edgelist(G_pos_validation, snakemake@output.validation_pos , delimiter="\t", data=False)
    nx.write_edgelist(G_neg_validation, snakemake@output.validation_neg , delimiter="\t", data=False)
