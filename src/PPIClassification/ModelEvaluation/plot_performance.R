...
auc_data$network_type <- sapply(auc_data$model, function(x) strsplit(x, "_")[[1]][2])
auc_data$selection <- sapply(auc_data$model, function(x) strsplit(x, "_")[[1]][3])
auc_data$partition <- sapply(auc_data$model, function(x) strsplit(x, "_")[[1]][4])

xy <- auc_data[auc_data$dataset=="goldensplit", c("delta_pr_auc", "delta_pr_auc_neg")]
auc_data = auc_data[auc_data$dataset!="goldensplit", ]

g <- ggplot(auc_data %>%filter(network_type == "directional"),
       aes(
         x=delta_pr_auc,
         y=delta_pr_auc_neg
         )) +
...
g <- ggplot(auc_data %>%filter(network_type == "undirectional"),
            aes(
              x=delta_pr_auc,
              y=delta_pr_auc_neg
            )) +
