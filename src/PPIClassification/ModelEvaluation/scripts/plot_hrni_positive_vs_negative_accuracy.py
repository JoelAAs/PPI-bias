...
lo = min(df["acc_pos"].min(), df["acc_neg"].min()) - 0.02
hi = max(df["acc_pos"].max(), df["acc_neg"].max()) + 0.02
...
for row_idx, random_value in enumerate(random_values):
    for col_idx, neg_limit in enumerate(neg_limits):
        ...
        if row_idx == len(random_values) - 1:
            ax.set_xlabel("Positive accuracy (HRNI)")
...
fig.suptitle("HRNI prediction accuracy: negative vs positive")
plt.tight_layout()
