...
lo = min(pivot["Negative_NO"].min(), pivot["Negative_HRNI"].min()) - 0.02
hi = max(pivot["Negative_NO"].max(), pivot["Negative_HRNI"].max()) + 0.02
...
for col_idx, neg_limit in enumerate(neg_limits):
    ...
    ax.set_xlabel("Jaccard (Negative_NO)")
    if col_idx == 0:
        ax.set_ylabel("Jaccard (Negative_HRNI)")
...
fig.suptitle(f"Prediction overlap (Jaccard, {snakemake.wildcards.pair_set} pairs): Negative_HRNI vs Negative_NO")
plt.tight_layout()
